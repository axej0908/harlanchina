...
public class Paper<T> implements Serializable {
     ...
     */
    public Paper(List<T> list, int totalCount, int pageSize, int currPage)
    {

        this.pageTotal=(int)Math.ceil((double)totalCount/pageSize);//总页数

        this.pageSize=pageSize;//一页多少条
        this.currentPage=currPage;//当前页数
        this.recordTotal=totalCount;//总的记录数

        //根据当前页面所在的页数和每页的大小 取值
        int startPtr=pageSize*(currPage-1);
        int FinalPtr=startPtr+pageSize;
        if(FinalPtr>=recordTotal)
        {
            //如果计算出来，当前取值范围超过list的最大值，则取值至末尾即可
            FinalPtr=recordTotal;
        }
        this.content=list.subList(startPtr,FinalPtr);//当前页面内容
