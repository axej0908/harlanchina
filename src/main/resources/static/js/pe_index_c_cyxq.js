$(function(){
    ...
    //商品类目面板的收放
    $(".good_area_list").on("click",".good_item_head",function(){
    	if($(this).siblings(".gd_cs_panel").css("display") == "block"){
    		$(this).find(".icon_btn").removeClass("icon_show").addClass("icon_hide");
    		$(this).siblings(".gd_cs_panel").slideUp()
    	}else{
    		$(this).find(".icon_btn").removeClass("icon_hide").addClass("icon_show");
    		$(this).siblings(".gd_cs_panel").slideDown()
    	}
    })
     ...
     var swiper_b = new Swiper('.swiper-container', {
	    nextButton: '.swiper-button-next',
	    prevButton: '.swiper-button-prev',
        slidesPerView: 4,
        spaceBetween: 10,
